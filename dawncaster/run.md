# Dawncaster Run 2 — Arcanist (Canto 1)

## Quick Status

| Field    | Value                                          |
|----------|------------------------------------------------|
| HP       | 55 / ? (healed significantly since node 6)    |
| Gold     | 20g (spent ~58g — shop/blacksmith?)            |
| Potions  | 0                                              |
| Level    | 2 (levelled up — talent picked)               |
| Weapon   | Forcewand — Deal 2 dmg, Gain 2 Chain. Chain. / Heals per Magic Action in deck |
| Energy   | 2 blue / turn (2 Intellect)                    |

---

## Deck (16 cards)

| Card | Copies | Cost | Notes |
|------|--------|------|-------|
| Shocking Grasp | 1 | 1 blue | Magic Lightning Action. Dmg = Chain count; 5+ Chain → 2 Shocked + Chain |
| Block | 1 | 0 gray | Basic defense. **Remove next Campsite** |
| Forcewand | 5 | 0 gray | Magic Lightning Basic Attack. **Remove 3-4 ASAP** |
| Surge of Intellect | 2 | — | Arcanist starting card. Effect unknown |
| Sageblast | 2 | 2 blue (1 after Quick Study?) | Magic Action. Dmg = Magic Actions in deck. Discard: Gain 🔵. |
| Stormbolt | 1 | ? | Magic Lightning Action. Deal 4 dmg. Continuity: Conjure Zap. Empower: Conjure Zap. Chain. |
| Gathering Storm | 1 | ? | Magic Lightning Action. Deal 4 dmg. Echo per turn in hand (0). Persistent. |
| Sanguine Scrying | 1 | ? | Action. Draw 2 cards. Gain 2 Bleeding. |
| Spellcraft | 1 | ? | Magic Action (picked node 2 of run 1 context — verify) |

> **Note:** Deck count confirmed 16 from screenshot. Card list is approximate — confirm exact contents.

> **Deck thinning priority:** Remove Block, then 3-4 Forcewands. Keep 2 Forcewands for Chain building.

---

## Talents

- **Quick Study** — Chose a card in deck, reduced its energy cost by 1. (Likely applied to Sageblast → 1 blue.)

---

## Meta Upgrades Active
- **Extra Experience** — level up faster → Arcane Mastery sooner
- **Bonus Health** — extra starting HP

---

## Run Log (newest first)

| # | Node | Outcome |
|---|------|---------|
| 11 | **Event: The Stew** | Options: Inspect / Kick the pot / Sample the stew / Leave. **Pending decision.** |
| 10 | **Unknown** | HP recovered from 27→55 (+28). Gold dropped 78→20 (-58). Deck grew to 16 cards. Likely Campsite + Shop/Blacksmith. |
| 9 | **Card Reward** | Offered: Call the Storm / Fan the Flames / Gathering Storm. Rec: pick Gathering Storm. HP 27, Gold 78, Deck 14. |
| 8 | **Card Reward** | Offered: Frostlance / Fleeting Thoughts / Sanguine Scrying. Rec: pick Sanguine Scrying. HP 27, Gold 73, Deck 14. |
| 7 | **Card Reward** | Offered: Aura of Winter / Siphon Magic / Stormbolt. Rec: pick Stormbolt. HP 27, Gold 71, Deck 13→14. |
| 6b | **Level Up** | Talent offered: Alchemist / Quick Study / Weapon Training. Rec: Quick Study (reduce Sageblast to 1 blue). |
| 6 | **Card Reward** | Skipped (Aura of Winter/Drake Familiar/First Aid Kit — all off-theme). Deck 11 cards. |
| 5 | **Campsite** | Removed Block ×1. Deck now 11 cards. 1 Block remains — remove next Campsite. |
| 4 | **Card Reward** | Skipped (Bat Familiar/Surgestone/Cauterize — all off-theme). |
| 3 | **Purple Shrine (Mirror)** | Mirrored Sageblast → 2× Sageblast, 8 Magic Actions in deck, each does 8 dmg. |
| 2 | **Card Reward** | Picked Sageblast. Dmg = Magic Actions in deck. Discard: Gain 🔵. |
| 1 | **Start** | Arcanist, Forcewand, 2 Intellect. Starting deck: Shocking Grasp ×1, Block ×2, Forcewand ×5, Surge of Intellect ×2 |

---

## Current Options

**Event: THE STEW**
- Inspect the stew
- Kick the pot over
- Sample the stew ← **Recommended** (55 HP = buffer to risk it; upside could be buff/card/HP)
- Leave

---

## Build Notes

**Direction:** Magic Action burst + Sageblast scaling. Every new Magic Action = +1 damage to all Sageblasts.

**Priority list:**
1. ~~Get Arcane Mastery at first level-up~~ — **Pending next level-up** (picked Quick Study this level)
2. Remove Block + 3-4 excess Forcewands at next Campsite/Blacksmith
3. Add Magic Actions liberally — each one buffs Sageblast
4. Keep 2 Forcewands for Chain building (feed Shocking Grasp)
5. **Heal to ≥85% before any boss** — lesson from run 1

**Current Magic Action count:** ~9 (Shocking Grasp, Sageblast×2, Stormbolt, Gathering Storm, Sanguine Scrying, Spellcraft, Surge×2 if they count)
→ Each Sageblast deals ~9 dmg; with Quick Study reducing one to 1 blue, can cast both per turn = **~18 damage from Sageblasts alone**

**Avoid:** Burn, Frozen, Holy card types.

---

## Open Questions
- [ ] What does **Surge of Intellect** do?
- [ ] Starting max HP this run (Bonus Health active)? — HP bar unclear, currently 55/?
- [ ] Did user actually pick Stormbolt, Sanguine Scrying, Gathering Storm? (based on deck count 16)
- [ ] What did -58 gold buy? (blacksmith removal? shop potion?)
- [ ] Quick Study applied to which card exactly?
- [ ] What is Forcewand's exact weapon power formula (heals per Magic Action — flat or scaling)?
